...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_saleshospital where billnumber = '$billnumber' and recordstatus <> 'DELETED' and companyanum = '$companyanum' and financialyear = '$financialyear'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_saleshospital where companyanum = '$companyanum' and financialyear = '$financialyear'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$consultationfee = $_REQUEST["consultationfees"];
	...
	$insurance = $_REQUEST["insuranceamount"];
	...
	$query102 = "select * from master_insurancecompany where insurancecompanyname = '$insurancecompany'";
	$exec102 = mysql_query($query102) or die ("Error in Query102".mysql_error());
	$res102 = mysql_fetch_array($exec102);
	$insurancecompanyanum = $res102["auto_number"];
	$insurancecompanycode = $res102["insurancecompanycode"];
	
	$query103 = "select * from master_tpa where tpaname = '$tpa'";
	$exec103 = mysql_query($query103) or die ("Error in Query102".mysql_error());
	$res103 = mysql_fetch_array($exec103);
	$tpaanum = $res103["auto_number"];
	$tpacode = $res103["tpacode"];
	
	
	$query3 = "insert into master_saleshospital (companyanum, billnumber, billdate, customertype, subtotal, customercode, customername, 
	address, location, city, state, pincode, phone, email, tinnumber, cstnumber, 
	roundoff, delivery, totalamount, totalquantity, billtype, cash, credit, insurance, online, creditcard, creditcardname, 
	...
	doctorname, dateofadmission, dateofdischarge, consultationfee, insurancecompany, insurancecompanyautonumber, insurancecompanycode, tpa, tpaautonumber, tpacode) 	
	values ('$companyanum', '$billnumber', '$billdate', '$customertype', '$subtotal', '$customercode', '$customername', 
	'$address', '$location', '$city', '$state', '$pincode', '$phone', '$email', '$tinnumber', '$cstnumber',  
	'$roundoff', '$delivery', 
	'$totalamount', '$totalquantity', '$billtype', '$cash', '$credit', '$insurance', '$online', '$creditcard', '$creditcardname', 
	'$footerline1', '$footerline2', '$footerline3', '$footerline4', '$footerline5', '$footerline6', '$subtotalafterdiscount', '$subtotalaftertax', 
	...
	'$subtotalamountdiscountpercent', '$subtotalamountdiscountamount', '$subtotalaftercombinediscount', 
	'$subtotaldiscountpercentapply1', '$subtotaldiscountamountapply1', '$subtotaldiscountamountonlyapply1', '$subtotaldiscountamountonlyapply2', 
	'$doctorname', '$dateofadmission', '$dateofdischarge', '$consultationfee', '$insurancecompany',  '$insurancecompanyanum', '$insurancecompanycode','$tpa', '$tpaanum', '$tpacode')";
	$exec3 = mysql_query($query3) or die ("Error in Query4".mysql_error());
	
	
	
	$query3 = "update master_transactionhospital set advanceremarks = 'ADJUSTED BY BILL NO.$billnumber' where customeranum = '$customeranum' and 
	advancestatus = 'ADVANCE PAYMENT' and advanceremarks = ''";
	$exec3 = mysql_query($query3) or die ("Error in Query3".mysql_error());
	
	$query3 = "select auto_number from master_saleshospital where companyanum = '$companyanum' and customercode = '$customercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query31 = "select * from master_itemhospital where itemcode = '$itemcode'"; 
			...
			$query5 = "select * from master_itemhospital where itemcode = '$itemcode'";
			...
			$query4 = "insert into hospitalsales_details (bill_autonumber, companyanum, 
			...
			*/
			...
			//To calc tax for cumulative discount given bill. This is applicabe to taxreport1payable1 also.
			$applicablediscountpercent1 = '0.00';
			if ($subtotalafterdiscount != $subtotalaftertax)
			{
				if ($subtotaldiscountpercentapply1 != '0.00')
				{
					$applicablediscountpercent1 = $subtotaldiscountpercentapply1;
				}
				if ($subtotaldiscountamountonlyapply2 != '0.00')
				{
					$applicablediscountpercent1 = $subtotaldiscountamountonlyapply2;
				}
				if ($applicablediscountpercent1 != '0.00')
				{
					$itemtotalamountdiscount = $itemtotalamount / 100;
					$itemtotalamountdiscount = $itemtotalamountdiscount * $applicablediscountpercent1;
					
					$itemtotalamount = $itemtotalamount - $itemtotalamountdiscount;
				}
			}
			...
			if ($itemcode != '')
			{
				$query7 = "insert into hospitalsales_tax (bill_autonumber, billnumber, billdate, itemanum, itemcode, itemname, 
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into hospitalsales_tax (bill_autonumber, billnumber, billdate, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
					updatedate, companyanum, companyname, financialyear) 
	...
	values ('$transactioncode', '$transactiondate', '$particulars1', '$customeranum', '$customername', 
	...
	if ($cashamount != '0.00')
	{
		...
		values ('$transactioncode', '$transactiondate', '$particulars', '$customeranum', '$customername', 
		'$transactionmode', '$transactiontype', '$transactionamount', '$cashamount', 
...
*/	
	...
	header ("location:sales1hospital.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=SALES BILL");
	exit;

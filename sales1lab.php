...
$query90 = "select count(auto_number) as masterscount from master_itemlab";
$exec90 = mysql_query($query90) or die ("Error in Query90".mysql_error());
$res90 = mysql_fetch_array($exec90);
$res90count = $res90["masterscount"];
if ($res90count == 0)
{
	header ("location:additem1lab.php?svccount=firstentry");
	exit;
}
...
include ("autocompletebuild_item1lab.php");
...
$query88 = "select count(auto_number) as cntanum from master_saleslab";// where lastupdate like '$thismonth%'";
...
if ($frm1submit1 == 'frm1submit1')
{
	...
	{
		//$query19 = "select auto_number,lastupdate from master_saleslab where auto_number = '$delbillautonumber' and billnumber = '$delbillnumber' and companyanum = '$companyanum' and recordstatus <> 'DELETED'";
		$query19 = "select auto_number,lastupdate from master_saleslab where billnumber = '$delbillnumber' and companyanum = '$companyanum' and financialyear = '$financialyear'";
		...
		while ($res19 = mysql_fetch_array($exec19))
		{
			...
			//$query15 = "update master_saleslab set recordstatus = 'DELETED' where auto_number = '$res19anum' and companyanum = '$companyanum'";
			...
			//$query16 = "update labsales_details set recordstatus = 'DELETED' where bill_autonumber = '$res19anum' and companyanum = '$companyanum'";
			...
			//$query17 = "update labsales_tax set recordstatus = 'DELETED' where bill_autonumber = '$res19anum' and companyanum = '$companyanum'";
			...
			$query18 = "update master_transactionlab set recordstatus = 'DELETED' where billnumber = '$delbillnumber' and companyanum = '$companyanum' and financialyear = '$financialyear'";
...
include ("sales1include1lab.php"); //handles all the sales insert operations
...
if ($delbillst == 'billedit' && $delbillnumber != '')
{
	$query41 = "select * from master_saleslab where billnumber = '$delbillnumber' and companyanum = '$companyanum' and financialyear = '$financialyear' and recordstatus <> 'deleted'";
...
<?php include ("js/sales1scripting1lab.php"); ?>
<?php include ("js/dropdownlist1scripting1.php"); ?>

<script type="text/javascript" src="js/salesinsertitem1lab.js"></script>
...
<script type="text/javascript" src="js/autocomplete_item1lab.js"></script> <!-- Drop Down List Holding File -->
<script type="text/javascript" src="js/autocomplete_itemsearch1.js"></script>
<script type="text/javascript" src="js/autocomplete_itemsearch3lab.js"></script> <!-- For mouse click event of item name drop down list. -->
<script type="text/javascript" src="js/autoitemsearch3lab.js"></script> <!-- For enter key selection and mouse click event of item name drop down list. -->
...
function funcPopupPrintFunctionCall()
{
	...
	{
	$query1print = "select * from master_printer_lab where defaultstatus = 'default' and status <> 'deleted'";
...
function loadprintpage1(varPaperSizeCatch)
{
	...
	if (varPaperSize == "A4")
	{
		window.open("print_bill1lab.php?printsource=billpage&&billautonumber="+varBillAutoNumber+"&&companyanum="+varBillCompanyAnum+"&&title1="+varTitleHeader+"&&billnumber="+varBillNumber+"","OriginalWindowA4<?php echo $banum; ?>",'width=722,height=950,toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=1,resizable=1,left=25,top=25');
	}
	if (varPaperSize == "A5")
	{
		window.open("print_bill1lab_a5.php?printsource=billpage&&billautonumber="+varBillAutoNumber+"&&companyanum="+varBillCompanyAnum+"&&title1="+varTitleHeader+"&&billnumber="+varBillNumber+"","OriginalWindowA5<?php echo $banum; ?>",'width=722,height=950,toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=1,resizable=1,left=25,top=25');
	}
}
...
function funcDoctorCommssionLostFocus1()
{
	if (document.getElementById("doctorcommission").value == "")
	{
		document.getElementById("doctorcommission").value = "0.00";
		return false;
	}
	if (isNaN(document.getElementById("doctorcommission").value))
	{
		alert ("Doctor Commission Can Only Be Numbers. Please Enter Proper Amount.");
		document.getElementById("doctorcommission").value = "0.00";
		document.getElementById("doctorcommission").focus();
		return false;
	}
}
...
function funcDefaultTax1() //Function to CST Taxes if required.
{
	...
	if (varDefaultTax != "")
	{
		...
		{
		?>
		window.location="sales1lab.php?defaulttax="+varDefaultTax+"&&delbillst=<?php echo $delBillSt; ?>&&delbillautonumber="+<?php echo $delBillAutonumber; ?>+"&&delbillnumber="+<?php echo $delBillNumber; ?>+"";
		<?php
		}
		else
		{
		?>
		window.location="sales1lab.php?defaulttax="+varDefaultTax+"";
		<?php
		}
		?>
	}
...
<form name="frmsales" id="frmsales" method="post" action="sales1lab.php" onKeyDown="return disableEnterKey(event)">
  ...
  <tr>
    ...
    <td width="99%" valign="top"><table width="980" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><table width="99%" border="0" align="left" cellpadding="2" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
            <tbody>
              <tr bgcolor="#011E6A">
                <td bgcolor="#CCCCCC" class="bodytext3"><strong>Lab Invoice </strong></td>
	...
	//$delBillSt = $_REQUEST["delbillst"];
			  ...
			  <tr>
				...
				<input name="customercode" id="customercode" value="<?php echo $res41customercode; ?>" style="border: 1px solid #001E6A" size="18" rsize="20" />
				<input type="button" name="customercodesearch" onClick="funcCustomerSearch2()" value="Search" style="border: 1px solid #001E6A">
				<!--<textarea name="deliveryaddress" cols="25" rows="3" id="deliveryaddress" style="border: 1px solid #001E6A"><?php //echo $res41deliveryaddress; ?></textarea>--></td>
                </tr>
			  <tr>
			    <td align="left" valign="middle"  bgcolor="#E0E0E0" class="bodytext3"><span class="style4"><!--Address 1 --></span><strong>Age &amp; Sex </strong></td>
			    ...
			    <td colspan="3" align="left" valign="top" ><span class="bodytext3"><strong><strong><strong>
			  ...
			  <tr>
			    ...
			    <td align="left" valign="top" ><input type="hidden" name="city1" id="city1" value="<?php echo $res41city; ?>" style="border: 1px solid #001E6A;text-transform: uppercase;"  size="10" /> 
		...
		<table id="newtable" style="BORDER-COLLAPSE: collapse" 
            ...
            <tbody id="tblrowinsert">
              <tr>
				...
				include ('sales_edit1listing1lab.php');
...
bgcolor="#F3F3F3" class="bodytext31"><div align="right"><strong>Nett Amount</strong></div></td>
              ...
              <tr>
                ...
                bgcolor="#CCCCCC" class="bodytext31"><div align="left"><span class="bodytext32"><strong><?php //echo $f21;?></strong></span>
                  ...
                  </span><strong>Doctor Commission </strong><span class="bodytext311">
                  <input name="doctorcommission" id="doctorcommission" onBlur="return funcDoctorCommssionLostFocus1()" style="border: 1px solid #001E6A; text-align:right" value="0.00" size="5" />
                ...
                bgcolor="#CCCCCC" class="bodytext31"><div align="right"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif">
                  ...
                  <input name="delbillnumber" id="delbillnumber" type="hidden" value="<?php echo $delbillnumber;?>">
					<input name="pageidentificationcontrol" id="pageidentificationcontrol" type="hidden" value="sales1lab">

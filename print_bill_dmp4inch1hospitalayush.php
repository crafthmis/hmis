...
$query3 = "select * from master_saleshospital where billnumber = '$billnumber' and recordstatus <> 'deleted' and companyanum = '$companyanum' and financialyear = '$financialyear'";
...
$res3customercode = $res3['customercode'];
$query4 = "select * from master_customer where customercode = '$res3customercode'";
$exec4 = mysql_query($query4) or die ("Error in Query4".mysql_error());
$res4 = mysql_fetch_array($exec4);
$res4gender = $res4['gender'];
$res4age = $res4['age'];
$res4address1 = $res4['address1'];
$res4city = $res4['city'];
$res4pincode = $res4['pincode'];
...
<script type="text/javascript">
      ...
      function funcPrint() {
         ...
         if (applet != null) {
			...
			//50 Char space.
            applet.append("\n");
            applet.append("<?php echo  '                                                       '.$billnumberprefix.$billnumber.$billnumberpostfix; ?>\n");
            applet.append("\n");
            applet.append("<?php echo  '                                                       '.substr($billdate, 0, 10); ?>\n");
            applet.append("\n");
            applet.append("<?php echo '      '.substr($customername, 0, 20).' / '.$res4gender.' / '.$res4age.'             '.$res3doctorname; // $customername; ?>\n");
			<?php
			}
			?>
            applet.append("<?php echo '      '.$res4address1; ?>\n");
            applet.append("<?php echo '      '.$res4city.' '.$res4pincode; ?>\n");
            <!--applet.append("ITEM        RATE QTY   TAX   AMOUNT\n");-->
            applet.append("\n");
            applet.append("\n");
            <!--applet.append("ITEM        RATE QTY   TAX   AMOUNT\n");-->
			<?php
			$serialnumber = "";
			$query4 = "select * from hospitalsales_details where bill_autonumber = '$billautonumber' and recordstatus <> 'deleted' and companyanum = '$companyanum' and financialyear = '$financialyear'";
			$exec4 = mysql_query($query4) or die ("Error in Query4".mysql_error());
			while ($res4 = mysql_fetch_array($exec4))
			{
				...
				//Customized For Ayush Hospital
				$printserialnumber = $serialnumber;
				$strlenprintserialnumber = strlen($printserialnumber);
				if ($strlenprintserialnumber == 3) { $printserialnumber = $printserialnumber; }
				if ($strlenprintserialnumber == 2) { $printserialnumber = ' '.$printserialnumber; }
				if ($strlenprintserialnumber == 1) { $printserialnumber = '  '.$printserialnumber; }
				
				$itemname = $res4["itemname"];
				$printitem = substr($itemname, 0, 40);
				
				$emptyspaceprintitem = '';
				$strlenprintitem = strlen($printitem);
				$totalblankspace3 = 40 - $strlenprintitem;
				for($i=1;$i<=$totalblankspace3;$i++)
				{
					$emptyspaceprintitem = $emptyspaceprintitem.' ';
				}
				$printitem = $printitem.$emptyspaceprintitem;
				...
				$printgridtext = $printserialnumber.'    '.$printitem.'         '.$printtotal; //.' '.$printtax.' '.$printtotal
			...
			*/
			...
			?>
			<?php
			$query41 = "select *, sum(taxamount) as sumtaxamount from hospitalsales_tax  where bill_autonumber = '$billautonumber' and recordstatus <> 'deleted' and companyanum = '$companyanum' and financialyear = '$financialyear' group by tax_autonumber, taxname order by auto_number";
            ...
            applet.append("\n");
            applet.append("\n");
            applet.append("<?php echo "                                   Total Amount:        ".$res3totalamount; ?>\n");
            ...
            applet.append("\n");
            applet.append("\n");
            <!--applet.append("<?php echo "PRINT END."; ?>\n");-->
            applet.append("\n");
            applet.append("\n");
            applet.append("\n");
            applet.append("\n");

...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$referencevalue = $splitdelimiter[8];
			...
			$itemunit = addslashes($itemunit);
			$referencevalue = addslashes($referencevalue);
			...
			if ($categoryname != '')
			{
				$query3 = "select * from master_categorylab where categoryname = '$categoryname'";
				$exec3 = mysql_query($query3) or die ("Error in Query3".mysql_error());
				$rowcount3 = mysql_num_rows($exec3);
				if ($rowcount3 != 0)
				{
					$query1 = "select * from master_itemlab where itemcode = '$itemcode'";
					...
					else
					{
						...
						{
							//$query5 = "select * from master_unitslab where unitname_abbreviation = '$itemunit'";
							...
							//{
								...
								$query2 = "insert into master_itemlab (itemcode, itemname, categoryname, 
								unitname_abbreviation, rateperunit, ipaddress, updatetime, 
								taxanum, taxname, purchaseprice, referencevalue) 
								values ('$itemcode', '$itemname', '$categoryname', '$itemunit', '$itemrate', '$ipaddress', '$updatedatetime', 
								'$taxanum', '$taxname', '$purchaseprice', '$referencevalue')";

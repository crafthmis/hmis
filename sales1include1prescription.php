...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_salesprescription where billnumber = '$billnumber' and recordstatus <> 'DELETED' and companyanum = '$companyanum' and financialyear = '$financialyear'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_salesprescription where companyanum = '$companyanum' and financialyear = '$financialyear'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$query3 = "insert into master_salesprescription (companyanum, billnumber, billdate, customertype, customercode, customername, 
	address, location, city, state, pincode, phone, email, tinnumber, cstnumber, 
	totalquantity, footerline3, footerline4, footerline5, footerline6,
	remarks, username, subtotaldiscounttotal, deliverymode, 
	ipaddress, billnumberprefix, billnumberpostfix, approvalstatus, 
	financialyear, doctorname, dateofadmission, dateofdischarge) 	
	values ('$companyanum', '$billnumber', '$billdate', '$customertype', '$customercode', '$customername', 
	'$address', '$location', '$city', '$state', '$pincode', '$phone', '$email', '$tinnumber', '$cstnumber',  
	'$totalquantity', '$footerline3', '$footerline4', '$footerline5', '$footerline6', 
	'$remarks', '$username', '$subtotaldiscounttotal', '$deliverymode', 
	'$lastupdateipaddress', '$billnumberprefix', '$billnumberpostfix', 
	'$approvalstatus', 
	'$financialyear', '$doctorname', '$dateofadmission', '$dateofdischarge')";
	$exec3 = mysql_query($query3) or die ("Error in Query4".mysql_error());
	
	$query3 = "select auto_number from master_salesprescription where companyanum = '$companyanum' and customercode = '$customercode' and lastupdate = '$lastupdate' and 
	billdate = '$billdate' and billnumber = '$billnumber' and financialyear = '$financialyear'";
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$morning = $_REQUEST["morning".$i];
			$noon = $_REQUEST["noon".$i];
			$night = $_REQUEST["night".$i];
			...
			$query4 = "insert into prescriptionsales_details (bill_autonumber, companyanum, 
			billnumber, itemanum, itemcode, itemname, itemdescription, rate, quantity, 
			free, discountpercentage, discountrupees, openingstock, closingstock, 
			discountamount, username, ipaddress, entrydate, itemtaxpercentage, itemtaxamount, 
			unit_abbreviation, financialyear, morning, noon, night) 
			values ('$billautonumber', '$companyanum', '$billnumber', '$itemanum', '$itemcode', '$itemname', '$itemdescription', '$itemmrp', '$itemquantity', 
			'$free', '$itemdiscountpercent', '$itemdiscountrupees', '$openingstock', '$closingstock', 
			'$discountamount', '$username', '$ipaddress', '$billdate', 
			'$itemtaxpercent', '$itemtaxamount', 
			'$itemunitabb', '$financialyear', '$morning', '$noon', '$night')";
			...
			if ($itemcode != '')
			{
				$query7 = "insert into prescriptionsales_tax (bill_autonumber, billnumber, billdate, itemanum, itemcode, itemname, 
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into prescriptionsales_tax (bill_autonumber, billnumber, billdate, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
					updatedate, companyanum, companyname, financialyear) 
...
*/	
	...
	header ("location:sales1prescription.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=SALES BILL");
	exit;

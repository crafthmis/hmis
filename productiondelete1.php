...
if ($task == 'delete' && $anum != '')
{
	$query19 = "select * from master_production where auto_number = '$anum' and companyanum = '$companyanum'";// and financialyear = '$financialyear'";
	...
	while ($res19 = mysql_fetch_array($exec19))
	{
		...
		//$query20 = "update master_stock set recordstatus='DELETED' where transactionmodule = 'production' and billnumber = '$delbillnumber' and companyanum = '$companyanum'";// and financialyear = '$financialyear'";
		//$exec20=mysql_query($query20) or die("Error in Query19".mysql_error());

	}
}

header ("location:productionreport1.php?task=deleted");

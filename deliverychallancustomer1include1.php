...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_dccustomer where billnumber = '$billnumber' and companyanum = '$companyanum'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_dccustomer where companyanum = '$companyanum'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$query3 = "insert into master_dccustomer (companyanum, billnumber, billdate, customertype, subtotal, customercode, customername, 
	...
	$query3 = "select auto_number from master_dccustomer  where companyanum = '$companyanum' and customercode = '$customercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query4 = "insert into dccustomer_details (bill_autonumber, companyanum, 
			...
			values ('$itemcode', '$itemname', '$billdate', 'DC CUSTOMER', 
			'BY DC CUSTOMER (BILL NO: $billnumber )', '$billautonumber', '$billnumber', '$itemquantity', '$remarks', 
			...
			if ($itemcode != '')
			{
				$query7 = "insert into dccustomer_tax (bill_autonumber, billnumber, itemanum, itemcode, itemname, 
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into dccustomer_tax (bill_autonumber, billnumber, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
					updatedate, companyanum, companyname) 
	...
	header ("location:deliverychallancustomer1.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=");
	exit;

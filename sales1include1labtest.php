...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_saleslabtest where billnumber = '$billnumber' and recordstatus <> 'DELETED' and companyanum = '$companyanum' and financialyear = '$financialyear'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_saleslabtest where companyanum = '$companyanum' and financialyear = '$financialyear'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$patientgender = strtoupper($_REQUEST["patientgender"]);
	$patientage = strtoupper($_REQUEST["patientage"]);
	$referredby = strtoupper($_REQUEST["referredby"]);
	$sampleid = strtoupper($_REQUEST["sampleid"]);
	$receiveddate = strtoupper($_REQUEST["receiveddate"]);
	$reporteddate = strtoupper($_REQUEST["reporteddate"]);
	...
	$query3 = "insert into master_saleslabtest (companyanum, billnumber, billdate, customertype, subtotal, customercode, customername, 
	...
	doctorname, dateofadmission, dateofdischarge, patientgender, patientage, referredby, 
	sampleid, receiveddate, reporteddate) 	
	...
	$query3 = "select auto_number from master_saleslabtest where companyanum = '$companyanum' and customercode = '$customercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query31 = "select * from master_itemlab where itemcode = '$itemcode'"; 
			...
			$resultvalue = $_REQUEST["resultvalue".$i];
			$referencevalue = $_REQUEST["referencevalue".$i];
			$testunit = $_REQUEST["testunit".$i];
			...
			$query5 = "select * from master_itemlab where itemcode = '$itemcode'";
			$exec5 = mysql_query($query5) or die ("Error in Query5".mysql_error());
			$res5 = mysql_fetch_array($exec5);
			$itemanum = $res5["auto_number"];
			$itemname = $res5["itemname"];
			$itemname = addslashes($itemname);
			$testunit = $res5['unitname_abbreviation']; //To avoid special character errors like Mu character.
			$itemcategoryname = $res5['categoryname'];
			
			$referencevalue = $res5['referencevalue']; //To avoid special character errors in js, db fetch is must.
			...
			if ($resultvalue != '')
			{
				...
				$query4 = "insert into labtestsales_details (bill_autonumber, companyanum, 
				...
				unit_abbreviation, financialyear, resultvalue, referencevalue, testunit, categoryname) 
				...
				'$itemunitabb', '$financialyear', '$resultvalue', '$referencevalue', '$testunit', '$itemcategoryname')";
				...
				*/	
				...
				if ($itemcode != '')
				{
					$query7 = "insert into labtestsales_tax (bill_autonumber, billnumber, billdate, itemanum, itemcode, itemname, 
				...
				while ($res72 = mysql_fetch_array($exec72))
				{
					...
					if ($rowcount72 != 0)
					{
						if ($itemcode != '')
						{
							$query8 = "insert into labtestsales_tax (bill_autonumber, billnumber, billdate, itemcode, itemname, itemrate, itemquantity, taxtype, 
							tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
							updatedate, companyanum, companyname, financialyear) 
...
*/	
	...
	header ("location:sales1labtest.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=SALES BILL");
	exit;

...
$searchresult = "";
	...
	*/
	...
	if ($searchresult == '')
	{
		$searchresult = ''.$itemcode.'||'.$itemname.'||'.$mrp.'||'.$taxname.'||'.$taxpercent.'||'.$taxanum.'||'.$itemdescription.'||'.$itemstock.'||'.$currentstock.'||'.$packageanum.'||'.$packagename.'||'.$salesprice.'||'.$manufacturername.'';
	}

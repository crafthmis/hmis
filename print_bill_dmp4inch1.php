...
$query3 = "select * from master_salespharmacy where billnumber = '$billnumber' and recordstatus <> 'deleted' and companyanum = '$companyanum' and financialyear = '$financialyear'";
...
$billtime = $res3['lastupdate'];
$billtime = substr($billtime, 11, 8);
...
$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
$res2 = mysql_fetch_array($exec2);
	$res2companyname = $res2["f9"];
	$res2addressline1 = $res2["f10"];
	$res2addressline2 = $res2["f11"];
	$res2addressline3 = $res2["f12"];
...
<script type="text/javascript">
      ...
      function funcPrint() {
         ...
         if (applet != null) {
			...
			$totalcharacterlength1 = 45;
			$totalblankspace1 = 45 - $strlen1;
            ...
            applet.append("\n");
            <?php
			$companyname = $res2companyname;
			$strlen2 = strlen($companyname);
			$totalcharacterlength2 = 45;
			$totalblankspace2 = 45 - $strlen2;
            ...
            <?php
			$address1 = $res2addressline1;
			$strlen3 = strlen($address1);
			$totalcharacterlength3 = 45;
			$totalblankspace3 = 45 - $strlen3;
			...
			{
			?>
            applet.append("DOCTOR  : <?php echo $res3doctorname; ?>\n");
            applet.append("PATIENT : <?php echo $customername; ?>\n");
			<?php
			}
			?>
            applet.append("BILL NO : <?php echo $billnumberprefix.$billnumber.$billnumberpostfix; ?>\n");
            applet.append("<?php echo 'DATE: '.substr($billdate, 0, 10).' '.$billtime; ?>\n");
            applet.append("----------------------------------------------------\n");
            applet.append("ITEM        RATE QTY   BATCH   EXPIRY   TAX   AMOUNT\n");
            applet.append("----------------------------------------------------\n");
			<?php
			$serialnumber = "";
			$query4 = "select * from pharmacysales_details where bill_autonumber = '$billautonumber' and recordstatus <> 'deleted' and companyanum = '$companyanum' and financialyear = '$financialyear'";
			...
			while ($res4 = mysql_fetch_array($exec4))
			{
				...
				$itemtaxpercentage = number_format($itemtaxpercentage, 2, '.', '');
				...
				$strlenprintbatchnumber = strlen($printbatchnumber);
				if ($strlenprintbatchnumber == 8) { $printbatchnumber = $printbatchnumber; }
				if ($strlenprintbatchnumber == 7) { $printbatchnumber = ' '.$printbatchnumber; }
				if ($strlenprintbatchnumber == 6) { $printbatchnumber = '  '.$printbatchnumber; }
				if ($strlenprintbatchnumber == 5) { $printbatchnumber = '   '.$printbatchnumber; }
				if ($strlenprintbatchnumber == 4) { $printbatchnumber = '    '.$printbatchnumber; }
				if ($strlenprintbatchnumber == 3) { $printbatchnumber = '     '.$printbatchnumber; }
				if ($strlenprintbatchnumber == 2) { $printbatchnumber = '      '.$printbatchnumber; }
				if ($strlenprintbatchnumber == 1) { $printbatchnumber = '       '.$printbatchnumber; }
				if ($strlenprintbatchnumber == 0) { $printbatchnumber = '        '.$printbatchnumber; }
				
				$printexpirydate = $expirydate;
				$strlenprintbatchnumber = strlen($printexpirydate);
				if ($strlenprintbatchnumber == 5) { $printexpirydate = $printexpirydate; }
				if ($strlenprintbatchnumber == 4) { $printexpirydate = ' '.$printexpirydate; }
				if ($strlenprintbatchnumber == 3) { $printexpirydate = '  '.$printexpirydate; }
				if ($strlenprintbatchnumber == 2) { $printexpirydate = '   '.$printexpirydate; }
				if ($strlenprintbatchnumber == 1) { $printexpirydate = '    '.$printexpirydate; }
				if ($strlenprintbatchnumber == 0) { $printexpirydate = '     '.$printexpirydate; }
				...
				$printgridtext = $printitem.' '.$printrate.' '.$printquantity.' '.$printbatchnumber.' '.$printexpirydate.' '.$printtax.' '.$printtotal
			?>
			...
			<?php 
				...
				$totalblankspace3 = $totalblankspace3 + 8 + 7; //FOR BATCH NUMBER
			...
			*/
			...
			?>
			<?php
			$query41 = "select *, sum(taxamount) as sumtaxamount from pharmacysales_tax  where bill_autonumber = '$billautonumber' and recordstatus <> 'deleted' and companyanum = '$companyanum' and financialyear = '$financialyear' group by tax_autonumber, taxname order by auto_number";

...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_saleslab where billnumber = '$billnumber' and recordstatus <> 'DELETED' and companyanum = '$companyanum' and financialyear = '$financialyear'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_saleslab where companyanum = '$companyanum' and financialyear = '$financialyear'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$doctorcommission = $_REQUEST['doctorcommission'];
	...
	$query3 = "insert into master_saleslab (companyanum, billnumber, billdate, customertype, subtotal, customercode, customername, 
	...
	doctorname, dateofadmission, dateofdischarge, patientgender, patientage, referredby, doctorcommission) 	
	...
	$query3 = "select auto_number from master_saleslab where companyanum = '$companyanum' and customercode = '$customercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query4 = "insert into labsales_details (bill_autonumber, companyanum, 
			...
			*/
			...
			if ($itemcode != '')
			{
				$query7 = "insert into labsales_tax (bill_autonumber, billnumber, billdate, itemanum, itemcode, itemname, 
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into labsales_tax (bill_autonumber, billnumber, billdate, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
					updatedate, companyanum, companyname, financialyear) 
...
*/	
	...
	header ("location:sales1lab.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=SALES BILL");
	exit;

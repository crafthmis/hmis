...
$query3 = "select * from master_purchaseorder where billnumber = '$billnumber' and recordstatus <> 'deleted' and companyanum = '$companyanum'";
...
<script type="text/javascript">
      ...
      function funcPrint() {
         ...
         if (applet != null) {
			...
			?>
            applet.append("PURCHASE ORDER NO: <?php echo $billnumberprefix.$billnumber.$billnumberpostfix; ?>\n");
            applet.append("<?php echo 'DATE: '.$billdate; ?>\n");
            applet.append("-------------------------------------\n");
            applet.append("ITEM        RATE QTY   TAX   AMOUNT\n");
			<?php
			$serialnumber = "";
			$query4 = "select * from purchaseorder_details where bill_autonumber = '$billautonumber' and recordstatus <> 'deleted' and companyanum = '$companyanum'";
			...
			*/
			...
			?>
			<?php
			$query41 = "select *, sum(taxamount) as sumtaxamount from purchaseorder_tax  where bill_autonumber = '$billautonumber' and recordstatus <> 'deleted' and companyanum = '$companyanum' group by tax_autonumber"; 

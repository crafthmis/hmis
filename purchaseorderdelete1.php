...
if ($task == 'delete' && $anum != '')
{
	$query19 = "select * from master_purchaseorder where auto_number = '$anum' and companyanum = '$companyanum'";// and financialyear = '$financialyear'";
	...
	while ($res19 = mysql_fetch_array($exec19))
	{
		...
		$query15 = "update master_purchaseorder set recordstatus = 'DELETED' where billnumber = '$delbillnumber' and companyanum = '$companyanum'";// and financialyear = '$financialyear'";
		$exec15 = mysql_query($query15) or die ("Error in Query15".mysql_error());
	
		$query16 = "update purchaseorder_details set recordstatus = 'DELETED' where billnumber = '$delbillnumber' and companyanum = '$companyanum'";// and financialyear = '$financialyear'";
		$exec16 = mysql_query($query16) or die ("Error in Query16".mysql_error());
	
		$query17 = "update purchaseorder_tax set recordstatus = 'DELETED' where billnumber = '$delbillnumber' and companyanum = '$companyanum'";// and financialyear = '$financialyear'";
...
header ("location:purchaseorderreport1.php?task=deleted");

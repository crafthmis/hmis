...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_dcsupplier where billnumber = '$billnumber' and companyanum = '$companyanum'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_dcsupplier where companyanum = '$companyanum'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$query3 = "insert into master_dcsupplier (companyanum, billnumber, billdate, suppliertype, subtotal, suppliercode, suppliername, 
	...
	$query3 = "select auto_number from master_dcsupplier  where companyanum = '$companyanum' and suppliercode = '$suppliercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query4 = "insert into dcsupplier_details (bill_autonumber, companyanum, 
			...
			if ($itemcode != '')
			{
				$query7 = "insert into dcsupplier_tax (bill_autonumber, billnumber, itemanum, itemcode, itemname, 
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into dcsupplier_tax (bill_autonumber, billnumber, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
					updatedate, companyanum, companyname) 
	...
	header ("location:deliverychallansupplier1.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=PURCHASE");
	exit;

...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_purchaseorder where billnumber = '$billnumber' and recordstatus <> 'DELETED' and companyanum = '$companyanum'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_purchaseorder where companyanum = '$companyanum'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$query3 = "insert into master_purchaseorder (companyanum, billnumber, billdate, suppliertype, subtotal, suppliercode, suppliername, 
	...
	lastupdate, ipaddress, billnumberprefix, billnumberpostfix, packaging, 
	...
	$query3 = "select auto_number from master_purchaseorder  where companyanum = '$companyanum' and suppliercode = '$suppliercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query4 = "insert into purchaseorder_details (bill_autonumber, companyanum, 
			...
			discountamount, username, ipaddress, entrydate, itemtaxpercentage, itemtaxamount, unit_abbreviation, billdate) 
			...
			'$itemtaxpercent', '$itemtaxamount', '$itemunitabb', '$billdate')";
			...
			values ('$itemcode', '$itemname', '$billdate', 'PURCHASEORDER', 
			'BY PURCHASEORDER (BILL NO: $billnumber )', '$billautonumber', '$billnumber', '$itemquantity', '$remarks', 
			...
			*/
			...
			if ($itemcode != '')
			{
				$query7 = "insert into purchaseorder_tax (bill_autonumber, billnumber, itemanum, itemcode, itemname, 
				itemrate, itemquantity, taxtype, 
				tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
				updatedate, companyanum, companyname, billdate) 
				...
				'$amountaftertax', '$ipaddress', '$updatedatetime', '$companyanum', '$companyname', '$billdate')";
				$exec7 = mysql_query($query7) or die ("Error in Query7".mysql_error());
			}
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into purchaseorder_tax (bill_autonumber, billnumber, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
	...
	header ("location:purchaseorder1.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=PURCHASEORDER");
	exit;

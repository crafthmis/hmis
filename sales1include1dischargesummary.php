...
if ($frm1submit1 == 'frm1submit1')
{
	if ($delbillst == 'billedit' && $delbillsummarynumber != '')
	{
		...
		if ($rowcount201 != 0) 
		{
			...
			$temparature = $_REQUEST["temparature"];
			...
			$temparature = addslashes($temparature);
			...
			doctorname, doctorcode, drugallergies, finaldiagnosis, chiefcomplaints, temparature, pulse,	bloodpressure,
			...
			medicalofficer, consultantofficer) 
			...
			header ("location:sales1dischargesummary.php?delbillst=billedit&&st=1&&delbillsummarynumber=$summarynumber&&summarynumber=$summarynumber");
			exit;
		}
		else
		{
			header ("location:sales1dischargesummary.php?delbillst=billedit&&delbillsummarynumber=$summarynumber&&st=2&&summarynumber=$summarynumber");
			exit;
		}
	}
}
...
if ($frm1submit1 == 'frm1submit1' && $delbillst == '' && $delbillsummarynumber == '')
{
	...
	*/
	...
	finaldiagnosis,	chiefcomplaints, temparature, pulse, bloodpressure, investigationdetails, treatmentgiven, diet, physicalactivity,
	...
	header ("location:sales1dischargesummary.php?src=frm1submit1&&st=1&&summarynumber=$summarynumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=SALES BILL");
	exit;
...
$exec2 = mysql_query($query2) or die ("Error in Query2".mysql_error());
$res2 = mysql_fetch_array($exec2);
//$res2summarynumber = $res2["summarynumber"];
$res2summarynumber = $res2["maxsummarynumber"];
if ($res2summarynumber == '')
{
	//$summarynumber = 'SBL00000001';
	$summarynumber = '1';
}
else
{
	$summarynumber = $res2["maxsummarynumber"];
	//$summarynumber = substr($res2summarynumber, 3, 8);
	//$summarynumber = intval($summarynumber);
	$summarynumber = $summarynumber + 1;
